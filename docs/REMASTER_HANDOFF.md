...
1. 战斗核心优先：先修玩家受击状态及攻击中断，接入现有 hurt 动画；再制作真正的多段空中连斩和收尾、强化各武器地面动作差异；实现玩家主动格挡和清晰的格挡/破防反馈，接入 PC/Xbox 提示。是否有精准弹反及其窗口须明确设计并验证，不能把盾卫减伤当作玩家格挡已完成。详见 docs/qa/remaster-0.6.0-combat-audit.md。当前地面有 3/4 段，空中重复同一 air 动画；玩家受击后实际播放 jump；盾卫减伤仍走普通受伤效果。此轮仅核查，无玩法修复或新发行包。
