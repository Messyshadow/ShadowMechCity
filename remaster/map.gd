...
const THEMES := ["city","temple","mine","water","factory","void","castle"]
const NAMES := ["机械中枢","遗迹神殿","废铁矿坑","腐化水道","蒸汽铸造厂","虚空要塞","暗影王城"]
var font: Font
...
func _ready() -> void:
	...
	for id in Rooms.ROOMS:
		var d:Dictionary=Rooms.ROOMS[id]
...
func center_player() -> void:
	if nodes.has(game.room_id):
		...
		pan.x=clampf(pan.x,minf(20,size.x-2332*zoom-20),20)
		pan.y=clampf(pan.y,minf(55,size.y-800*zoom-15),55)
	queue_redraw()

func fit_all() -> void:
	zoom=minf((size.x-40)/2332.0,(size.y-75)/800.0);pan=Vector2(20,50);queue_redraw()
...
func _draw() -> void:
	...
	for id in nodes:
		var d:Dictionary=Rooms.ROOMS[id];var visited:bool=Reforged.visited.has(id)
