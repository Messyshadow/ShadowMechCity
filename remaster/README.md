...
当前开发试玩版 **0.7.0**：进化分页下设暗影/机械技能点分支，两种限时变身、两种临时召唤和强化；保留探索模块、收藏、Xbox 自动切换、七武器及伙伴。共 39 技能节点，1004 项检查通过。详见 [发布验收记录](qa/RELEASE_0_7_0.md) 和 [原设计与新需求对照表](../docs/REMASTER_ROADMAP.md)。完整重制仍在进行。
...
- 最新本机包：`build/Reforged/ShadowMechCityReforged.exe`，版本 **0.7.0**。
- 本机发布目录：`E:\Godot\release\暗影机械城重制版`；发布包：`E:\Godot\release\暗影机械城重制版-0.7.0.zip`。
...
| 3D 资产 | 65 个 Blender 模型，包括猎魂者、商人、巡线员、五类普通敌人、七位 Boss、四种伙伴、建筑与装备；10 张 512px 装备渲染图 |
...
| 技能分页 | 战斗、身法、进化；共 39 节点，前置连线、详情和 3D 预览；暗影/机械进化各有 3 被动、变身、召唤、时长强化，旧技能保持有效 |
...
0.7.0 游戏检查共 **1004 项通过**（此前 934 项回归 + 70 项进化专项）。新增检查入口 `tests/test_evolution.gd` 验证学习、实际战斗数值、限时清理、共享冷却、存档兼容、输入隔离和技能树。实机截图另行验证；历史发布记录保留在 `qa/`。
