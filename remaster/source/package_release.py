...
def main():
    ...
    (BUILD / "开始前请读.txt").write_text(
        ...
        "T → 进化：暗影 / 机械技能点分支；学习后 Z 限时变身、V 临时召唤。\n"
        "Xbox 按住上方向再按 Y 变身、L3 召唤；右上角显示倒计时。\n"
        "七种武器可用 Q 切换，或在 T 技能树中直接装备。\n\n"
        "F 投掷脉冲炸弹，需先在熔岩腔穴取得模块；Xbox 为下方向 + Y。\n"
        "N 打开任务后可查看城市档案、探索模块位置和补给规则。\n"
        "六枚生命碎片各 +10 最大生命，七枚记忆核心恢复城市档案。\n\n"
        "默认 PC 按键；检测到 Xbox / XInput 手柄连接后自动切换提示，断开后切回 PC。\n"
        "Xbox：A 跳跃、X 普攻、Y 技能、B 冲刺；LB 换武器、RB 交互；\n"
        "LT 治疗、RT 装填；View 打开功能页，Menu 暂停；L3/R3 伙伴。\n"
        "菜单 A 确认、B 返回，LB/RB 切功能页。实体手柄尚待实测。\n\n"
        "首次与中央车站的商人赫克对话可领取吸血护符。\n"
        "在同步终端按 E 保存并补给；死亡回到最后激活的存档点。\n"
        "背包可强化装备，出售的装备可从商人回购栏买回。\n\n"
        "F11 切换全屏；设置可调整音量、亮度、镜头震动与教学。\n"
        "与车站的巡线员莉娅交谈，可接取晨曦温室的引航灯委托。\n\n"
