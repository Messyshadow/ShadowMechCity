...
func _ready() -> void:
	...
	for i in range(16):
		var v := AudioStreamPlayer.new(); add_child(v);v.bus="Effects"; voices.append(v)
	ambient = AudioStreamPlayer.new(); add_child(ambient)
	ambient.stream = sounds.ambience; ambient.volume_db = -24;ambient.bus="Ambience"
	ambient.finished.connect(func(): ambient.play())
	if playback_enabled:ambient.play()
	music=AudioStreamPlayer.new();add_child(music);music.volume_db=-22;music.bus="Music"
	music.finished.connect(func():music.play())

func set_region(region: String, boss := false) -> void:
	var next:="boss" if boss else "city" if region=="dawn" else region
...
func _process(_delta: float) -> void:
	...
	AudioServer.set_bus_mute(0,Reforged.mute or float(Reforged.settings.master)<=0)
