...
func _init() -> void:
	...
	_check(source.contains("func _make_portal_maintenance_details"), "down portals need floor-mounted maintenance shaft details")
	_check(source.contains("func _make_portal_solid_panel"), "maintenance shafts need solid tiled rim panels instead of only line art")
	...
	_check(source.contains("root.position = Vector2(d[\"p\"], b[3] - 8)"), "portal rim must be embedded in the floor instead of floating above it")
